      ...
      <div className="bg-zinc-950/90 backdrop-blur-md border border-zinc-800 rounded-xl shadow-2xl w-64 overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-3 border-b border-zinc-800/80 bg-zinc-900/80">
          <h2 className="text-sm font-medium text-white flex items-center gap-2">
            <Activity size={14} className={selectedPlanet === 'Moon' ? 'text-indigo-400' : 'text-emerald-400'} />
        ...
        <div className="p-3 text-xs text-zinc-300 space-y-3">
          <div className="bg-zinc-900/50 p-2 rounded border border-zinc-800/50">
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-2 font-mono text-[10px]">
              ...
              {planetData.elongation !== undefined && <><span className="text-zinc-500">Elongasi:</span> <span>{planetData.elongation.toFixed(1)}°</span></>}
              {planetData.angle !== undefined && planetData.angle > 0 && <><span className="text-zinc-500">Sudut (S):</span> <span>{planetData.angle.toFixed(1)}°</span></>}
              <span className="text-zinc-500">Rasi Bintang:</span> <span className="text-yellow-400">{planetData.constellation}</span>
            </div>
          </div>
          ...
          {planetData.name === 'Moon' && (
            <div className="bg-zinc-900/50 p-2 rounded border border-zinc-800/50">
              <div className="text-indigo-400 font-medium mb-1.5 border-b border-zinc-800 pb-1">Detail Bulan</div>
              <div className="grid grid-cols-2 gap-x-2 gap-y-2 font-mono text-[10px]">
                <span className="text-zinc-500">Iluminasi:</span> <span>{planetData.illumination.toFixed(1)}%</span>
